    ...
    <div style={{ background: "var(--card-bg)" }}>
      <section className="py-16 px-6 border-b" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
        <div className="max-w-4xl mx-auto">
          <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: "var(--teal)" }}>About</p>
          <h1 className="text-[clamp(2.25rem,1.75rem+2.5vw,3.5rem)] font-bold tracking-tighter text-balance mb-4" style={{ color: "var(--text)" }}>易澤居 EasyRent</h1>
          <p className="text-base leading-relaxed max-w-[55ch] text-pretty" style={{ color: "var(--text-muted)" }}>More than an agent — your partner in Hsinchu.</p>
        </div>
      </section>
      ...
      <section className="py-16 px-6" style={{ background: "var(--card-bg)" }}>
        <div className="max-w-4xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: "var(--teal)" }}>Mission</p>
              <h2 className="text-2xl font-bold tracking-tight mb-5" style={{ color: "var(--text)" }}>Every expat in Hsinchu<br />deserves a home that works</h2>
      ...
      <section className="py-16 px-6" style={{ background: "var(--surface)" }}>
        <div className="max-w-4xl mx-auto">
          <div className="max-w-xl mb-10">
            <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: "var(--teal)" }}>Differentiators</p>
            <h2 className="text-2xl font-bold tracking-tight text-balance" style={{ color: "var(--text)" }}>Why we're different</h2>
          </div>
      ...
      <section className="py-16 px-6" style={{ background: "var(--card-bg)" }}>
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest mb-5" style={{ color: "var(--teal)" }}>Service Areas</p>
            <div className="divide-y rounded-2xl overflow-hidden border" style={{ borderColor: "var(--border)" }}>
              {[{ area: "Zhubei City", detail: "HSR district · Zhungzhi Road" }, { area: "Hsinchu City", detail: "Guanpu zone · near HSIP" }, { area: "Other areas", detail: "Hukou · Zhudong · on request" }].map((a) => (
                <div key={a.area} className="flex items-center justify-between px-5 py-4" style={{ background: "var(--card-bg)" }}>
                  <span className="text-sm font-semibold" style={{ color: "var(--text)" }}>{a.area}</span>
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest mb-5" style={{ color: "var(--teal)" }}>Languages</p>
            <div className="space-y-3">
              {[{ flag: "🇬🇧", lang: "English", note: "All services available" }, { flag: "🇯🇵", lang: "日本語", note: "全サービス対応" }, { flag: "🇹🇼", lang: "繁體中文", note: "主要服務語言" }].map((l) => (
                <div key={l.lang} className="flex items-center gap-4 px-5 py-4 rounded-xl border" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
